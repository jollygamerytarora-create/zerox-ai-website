...
const buildPath = (
  shape: NonNullable<TextLoopProps["shape"]>,
  curviness: number,
  ribbonWidth: number,
): string => {
...
/**
 * React Bits — TextLoop (adapted, TypeScript).
 * Phrase repeated along an SVG curve, scrolling forever.
 */
export default function TextLoop({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: re-measures path metrics when visual inputs change
  useLayoutEffect(() => {
  ...
  useEffect(() => {
    ...
    // alternate frames halves that cost, visually indistinguishable.
    let frame = 0;
    const tween = gsap.to(state, {
      ...
      repeat: -1,
      onUpdate: () => {
        if ((frame++ & 1) === 0) apply(state.offset);
      },
    });
