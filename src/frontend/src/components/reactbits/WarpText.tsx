...
const buildTextCanvas = ({
  ...
  let fontSizePx = Number.parseFloat(computed.fontSize) || 96;
...
const syncUniforms = (program: Program, props: WarpTextProps) => {
...
/**
 * React Bits — WarpText (adapted, TypeScript).
 * Headline rendered to a WebGL texture and refracted through
 * animated glass; the pointer lenses and bends the letters.
 */
export default function WarpText({
  ...
  const contextRef = useRef<{ program: Program; rasterize: () => void } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const rasterize = async () => {
      ...
      const textCanvas = buildTextCanvas({
        ...
        height: rect.height,
        dpr,
        props: propsRef.current as never,
      });
